...
class EvalModel:
    ...
    def get_outputs(
        ...
        with torch.inference_mode():
            with self.autocast():
                outputs = self.model.generate_lightning(
                    self._prepare_images(batch_images).to(self.device, non_blocking=True),
    ...
    def encode_vision_x(self, image_tensor: torch.Tensor):
        self.model._encode_vision_x(image_tensor.to(self.device))

    def uncache_media(self):
        self.model.uncache_media()

    def cache_media(self, input_ids, vision_x):
        self.model.cache_media(input_ids=input_ids, vision_x=vision_x)
